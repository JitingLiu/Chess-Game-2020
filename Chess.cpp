...
const char chessFlag1 = 'o';      //玩家1或电脑的棋子标志
...
void Chess::init_vector() //容器初始化
{
    ...
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < N; ++j)
        {
            ...
            else if (chessBoard[i][j] == chessFlag1) //电脑走的位置
            {    
                vec_all.push_back(pos);
            }
...
void Chess::AI_chess(Position& pos, char flag) //电脑走
{      
    init_vector();
...
int ratio = 1.2;  // 进攻的系数   大于1 防守型，  小于1 进攻型
