...
void GameWindow::mouseReleaseEvent(QMouseEvent* event) //下棋动作
{
    ...
    if (game->judge_postition(player2) == 1){ //判断坐标是否合法
        game->take_step(player2, 2, chessFlag2); //玩家走
    ...
    lasty = player2.col;
    // 判断输赢
    if (is_win(player2, 2, chessFlag2)){ //玩家
    ...
    lasty = player1.col;
    // 判断输赢
    if (is_win(player1, 1, chessFlag1)){ //AI
        game->Init_chessboard(); //清空
        update(); //再来一局
    }
...
{
    moveX = (event->position().y()-40)/40;
    moveY = (event->position().x()-20)/40;
    update(); //更新后就重画
}
...
void GameWindow::paintEvent(QPaintEvent* event)
{
    ...
    for(int i=0; i < 15; ++i)
    {
        for(int j=0; j <15; ++j)
        {
            if(game->get_coordinate(i,j) == chessFlag1)// 黑子,AI
            {
